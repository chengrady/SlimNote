<template>
  <div class="json-toolbar">
    <span class="json-toolbar-meta">JSON</span>
    <button class="json-toolbar-button" type="button" @click="$emit('format')" title="格式化 (Ctrl+Shift+F)">
    ...
    <button class="json-toolbar-button" type="button" @click="$emit('minify')" title="压缩 (Ctrl+Shift+M)">
    ...
    <button class="json-toolbar-button" type="button" @click="$emit('repair')" title="修复 (Ctrl+Shift+R)">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
    </button>
    <button class="json-toolbar-button" type="button" @click="$emit('unescape')" title="去除转义">
    ...
    <button class="json-toolbar-button" type="button" @click="$emit('expand-all')" title="全部展开">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>
    </button>
    <button class="json-toolbar-button" type="button" @click="$emit('collapse-all')" title="全部折叠">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 14h6v6M20 10h-6V4M14 10l7-7M10 14L3 21"/></svg>
    </button>
    <div class="json-toolbar-separator"></div>
    <button class="json-toolbar-button" type="button" @click="$emit('open-converter')" title="格式转换">
    ...
    <button class="json-toolbar-button" type="button" @click="$emit('open-codegen')" title="代码生成">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
    </button>
    <button class="json-toolbar-button" type="button" @click="$emit('open-diff')" title="JSON Diff">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
    </button>
    <button class="json-toolbar-button" type="button" @click="$emit('export-image')" title="导出图片">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
    </button>
    <div class="json-toolbar-separator"></div>
    <button class="json-toolbar-button" type="button" :class="{ active: showTree }" @click="$emit('toggle-tree')" title="切换树形视图 (Ctrl+Shift+T)">
